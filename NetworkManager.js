"use strict";
function __safeNetworkInterfaces(os) {
  try {
...
class NetworkManager extends events_1.EventEmitter {
    constructor(options) {
        ...
        this.initPromise = new Promise(resolve => {
            this.getCurrentNetworkInterfaces().then(map => {
                ...
                const otherInterfaces = Object.keys(__safeNetworkInterfaces(os_1.default));
    ...
    async getCurrentNetworkInterfaces() {
        ...
        const networkInterfaces = __safeNetworkInterfaces(os_1.default);
    ...
    static resolveInterface(address) {
        let interfaceName;
        outer: for (const [name, infoArray] of Object.entries(__safeNetworkInterfaces(os_1.default))) {
    ...
    static assumeNetworkInterfaceNames() {
        ...
        const names = [];
        Object.entries(__safeNetworkInterfaces(os_1.default)).forEach(([name, infos]) => {
    ...
    static getLoopbackInterface() {
        for (const [name, infos] of Object.entries(__safeNetworkInterfaces(os_1.default))) {
            for (const info of infos !== null && infos !== void 0 ? infos : []) {
                if (info.internal) {
                    return name;
                }
